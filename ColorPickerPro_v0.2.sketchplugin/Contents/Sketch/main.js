...
function buildColorContentView() {
    var itemHeight = 40, headerHeight = 24, padding = 8, sidePadding = 12;
    ...
    for (var i = 0; i < buttonPositions.length; i++) {
        ...
        rowBtn.setCOSJSTargetFunction(function (sender) { thePopup.selectItemAtIndex(sender.tag()); });
...
function refreshPanelIfNeeded() {
    ...
    if (!collectCurrentSelectionColors()) return;
    ...
    thePopup.setFrame(NSMakeRect(margin, dialogHeight - margin - scrollHeight - gapBetween - 26, panelWidth, 26));
}
...
function showSelectionColors(context) {
    ...
    if (!collectCurrentSelectionColors()) {
        UI.message('Please select layers with colors');
        return;
    }
    lastSelectionIds = getSelectionIdString();
